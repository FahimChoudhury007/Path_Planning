...
def bicycle_robot_model(q, u, L=0.3, dt=0.01):
    ...
    """
    ## Defining Input Variables for our robot.
    u1 = u[0]
    u2 = u[1]
    ## Dynamics of our robot.
    q_dot = vertcat(cos(q[2]),sin(q[2]), tan(q[3])/L,0)*u1 + vertcat(0,0,0,1)*u2 
    q_next = q + q_dot * dt
...
def initial_cond(q_start, q_goal, n):
    ...
    initial_pos = np.array([q_start[0],q_start[1]])
    goal_pos = np.array([q_goal[0],q_goal[1]])

    spline_x = CubicSpline([0,1],[initial_pos[0],goal_pos[0]])
    spline_y = CubicSpline([0,1],[initial_pos[1],goal_pos[1]])

    interpolated_x = spline_x(t)
    interpolated_y = spline_y(t)
...
def objective_func(q, u, q_goal, Q, R, P):
    ...
    for i in range(n):
        ...
        for k in range(4):
            for j in range(4):
                term += (qi[k] - q_goal[k])*Q[k,j]*(qi[j]-q_goal[j])
        # obj += mtimes((qi-q_goal).T, mtimes(Q, qi-q_goal)) + mtimes(ui.T, np.dot(R, ui))
        obj += term

        u_term = 0
        for k in range(2):
            for j in range(2):
                u_term += (ui[k])*R[k,j]*(ui[j])
    ...
    for i in range(4):
        for j in range(4):
            term_last = (q_last[i]-q_goal[i]).T * P[i,j] * (q_last[j]-q_goal[j])
    
    # obj += mtimes((q_last-q_goal).T, mtimes(P, q_last-q_goal))
    obj += term_last
   ...
   # State constraints
    ...
    for obj in obs_list:
        ...
        for t in range(q.shape[1]):
            constraints.append((obj_x-q[0,t])**2 +  (obj_y-q[1,t])**2>(obj_r + 0.01)**2) # Define the obstacle constraints.

    # Initial and final state constraints
    n = q.shape[1] - 1
    for i in range(4):
        constraints.append(q[i,0] == q_start[i]) # Constraint on start state.
        constraints.append(q[i,n]== q_goal[i]) # Constraint on final state.
