...
class Plan(object):
    ...
    def chain_paths(self, *paths):
        ...
        def chain_two_paths(path1, path2):
            ...
            # Wrong with the open loop inputs
...
class BicycleConfigurationSpace(ConfigurationSpace):
    ...
    def __init__(self, low_lims, high_lims, input_low_lims, input_high_lims, obstacles, robot_radius):
        ...
        np.random.seed(42)
        self.probability = 0.9 # Need to tune this variable
        self.TOTAL_TIME = 1
        primitive_u1 = np.arange(-1.5, 1.6, 0.2).tolist()
        primitive_u2 = np.arange(-0.6, 0.6, 0.1).tolist()
        self.primitives = list(itertools.product(primitive_u1, primitive_u2))
    ...
    def distance(self, c1, c2):
        ...
        """
        abs_theta_diff = np.abs(c1[2] - c2[2])
        min_theta_diff = min(abs_theta_diff, np.pi - abs_theta_diff)
        euclidean_metric =  ((c1[0]-c2[0])**2 + (c1[1]-c2[1])**2 + min_theta_diff**2)**1/2
    ...
    def sample_config(self, *args):
        ...
        """
        goal = args[0]
        return np.random.uniform(self.low_lims, self.high_lims).reshape((self.dim,)) if np.random.rand() >= self.probability else goal
    ...
    def check_collision(self, c):
        ...
        for obstacle in self.obstacles:
            ...
            euclidean_distance = ((c[0] - obstacle_x)**2 + (c[1] - obstacle_y)**2)**0.5
            if euclidean_distance <= obstacle_radius + self.robot_radius:
                return True
        return False
    ...
    def check_path_collision(self, path):
        ...
        """
        target_positions = path.positions
        open_loop_inputs = path.open_loop_inputs
        for i in range(len(target_positions)):
            target_pos = target_positions[i]
            target_input = open_loop_inputs
            if self.check_collision(target_pos):
                return True
            if self.check_constraints(target_pos, target_input):
                return True
        return False
    
    # If the args exceeds or fails to the contraints, return true
    def check_constraints(self, positions, inputs):
        if np.any(positions > self.high_lims) or np.any(positions < self.low_lims):
            return True
        if np.any(inputs > np.array(self.input_high_lims)) or np.any(inputs < np.array(self.input_low_lims)):
            return True
        return False
    ...
    def local_plan(self, c1, c2, dt=0.01):
        ...
        # Go through each primitive over total amount of time
        for primitive in tqdm(self.primitives):
            u1, u2 = primitive
            positions = [np.copy(c1)]
            current_position = np.copy(c1)
            
            # Over given time
            for _ in np.arange(0, self.TOTAL_TIME, dt):
                # Dynamics
                theta = current_position[2]
                phi = current_position[3]
                v1 = np.array([np.cos(theta), np.sin(theta), np.tan(phi)/self.robot_length, 0])
                v2 = np.array([0, 0, 0, 1])
                velocity = u1 * v1 + u2 * v2
                current_position = current_position + velocity * dt # Update current position
                positions.append(current_position)

            current_plan = Plan(np.arange(0, len(positions) * dt, dt), np.array(positions), np.tile([u1, u2], (len(positions), 1)))    
            # If the new position and primitive violates the contraints, do not build on top of this plan
            if self.check_path_collision(current_plan):
                continue

            latest_position = current_plan.end_position()
