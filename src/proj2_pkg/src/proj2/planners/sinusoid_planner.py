...
from .configuration_space import Plan, BicycleConfigurationSpace #remove the . when running from this file without ros
...
class SinusoidPlanner():
    ...
    def steer_x(self, start_state, goal_state, t0 = 0, dt = 0.01, delta_t = 2, alternate=False):
        """
        ...
        if (alternate):
            ...
            g = lambda alpha: alpha/np.sqrt(1-alpha*alpha)

            #do binary searhc
            epsilon = 0.05 #determines if a1 results in a G close enough to delta_x
            a1_lower = -10 #lower bound for the range of a1
            a1_upper = 10 #upper bound for the range of a1
            while True:
                a1 = (a1_upper + a1_lower)/2
                
                phi_fn = lambda t: (a2/(2*omega))*np.sin(2*omega*t) + start_state_v[1]
                inner_integrand = lambda t: f(phi_fn(t))*a1*np.sin(omega*t)
                outer_integrand = lambda t: g(quad(inner_integrand, 0, t)[0] + start_state_v[2])*np.sin(omega*t)
                beta1 = (omega/np.pi)*quad(outer_integrand, 0, delta_t)[0]

                G = (np.pi/omega)*a1*beta1
                if (abs(delta_x - G) < epsilon):
                    break
                elif (G < delta_x):
                    a1_lower = a1 # search upper half next
                else:
                    a1_upper = a1 # search lower half next
    ...
    def steer_phi(self, start_state, goal_state, t0 = 0, dt = 0.01, delta_t = 2, alternate=False):
        """
    ...
    def steer_alpha(self, start_state, goal_state, t0 = 0, dt = 0.01, delta_t = 2, alternate=False):
        """
    ...
    def steer_y(self, start_state, goal_state, t0 = 0, dt = 0.01, delta_t = 2, alternate=False):
        """
        ...
        if alternate:
            ...
            #do binary searhc
            epsilon = 0.05 #determines if a1 results in a G close enough to delta_y
            a1_lower = -10 #lower bound for the range of a1
            a1_upper = 10 #upper bound for the range of a1
            while True:
                a1 = (a1_upper + a1_lower)/2
